/**
 *
 * Remove the num_in_rank, mum_in_suit, and card_exists arrays from the
 * poker.c program of Section 10.5. Have the program store the cards in
 * a 5 x 2 array instead. Each row of the array will represent a card.
 * For example, if the array is named hand, then hand[0][0] will store
 * the rank of the first card and hand [0][1] will store the suit of the
 * first card.
 *
 */
#include <stdbool.h>
...
void analyze_hand(void) {
   ...
   for (int card = 0; card < NUM_CARDS; card++) {
      for (rank = card + 1; rank < NUM_RANKS; rank++) {
         if (hand[card][0] == hand[rank][0])
            same_ranks++;
      }
   }
    ...
    * check for straight:
    *
    *    in read_cards function, we set rank values (as integers)
    *    according to each corresponding rank_ch values and
    *    we summed up into the sum_ranks variable.
    *
   ...
   if (same_ranks == 4)
      four = true;
   else if (same_ranks == 3)
      three = true;
   else
      pairs = same_ranks; // 1 or 2
}
